...
public class Identify_Elements_pwiz {
	...
	Activities_cmp acts_cmp= new Activities_cmp();
	Cmp_trade_Name cmp_tname=new Cmp_trade_Name();
	Cmp_period cmperiod=new Cmp_period();
	Financial_shares fin_share=new Financial_shares();
	Reserve_Trade_Name tnames=new Reserve_Trade_Name();
	String r_name;
	List<WebElement> resv_name_list;
	String Contract_ID;
	GET_ECR_Data multi_arrs;
	Data_Layer_PreWiz data_db=new Data_Layer_PreWiz();
	int selected_saudi_index;
	...
	String[] usernames= {"1004470629"};//,"2285625097","Sagiauser123","khaliji_01"};
	...
	{
		selected_saudi_index=rand.nextInt(retrived_IDs.length-1);
		
		returned_username_index=login_temp(retrived_IDs[selected_saudi_index],passwords[repeat_time]);
	...
	//	export.print_results_HelpMe(help_me_values,0);
		...
		try
		{
			r_name=tnames.reserve_sugg_name(driver, wait);
		}catch(Exception c)
		{
		...
		System.out.println("Reserved name is "+r_name);
		WebElement main_pg=wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//li[@role='presentation']/a[@href='/Unified/Welcome']")));
		main_pg.click();
...
//		{
		...
		Finish_Step(driver,wait);
		cmp_tname.Select_TM_pre(driver, wait, r_name);
		//cmp_tname.Select_TM_Quick(driver, wait);
		Finish_Step(driver, wait);
		cmperiod.Select_cmp_period_fiscal(driver, wait);
		Finish_Step(driver, wait);
		fin_share.insert_fin_shares(driver, wait);
		Save_continue_later(driver, wait);
		Contract_ID=get_contract_ID(driver, wait);
		 
		export.print_results_PreWizard(activities_to_print, results_Str,results_ex_Str,repeat_time++, Cmp_Type,returned_username_index, cpt_amount,Contract_ID);
...
//		}
	...
	{
		JavascriptExecutor jse = (JavascriptExecutor)driver;
		auto_picked_company = 1;//rand.nextInt(3) + 1;
	...
	public String[] verify_results()
	{
		...
		publish_Fees_Element=     wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[contains(text(),'النشر')]/../../div/span[text()='ريال']/../strong")));
		...
		System.out.println(main_CR_Fees_Element.getText().replaceAll("\\W", ""));
		
		main_CR_Fees=Double.valueOf(main_CR_Fees_Element.getText().replaceAll("\\W", ""));
		//coc_Fees=Double.valueOf(coc_Fees_Element.getText());
		publish_Fees=Double.valueOf(publish_Fees_Element.getText().replaceAll("\\W", ""));
		total_Fees=Double.valueOf(total_Fees_Element.getText().replaceAll("\\W", ""));
		vat_Fees=Double.valueOf(vat_Fees_Element.getText().replaceAll("\\W", ""));
		total_plus_vat=Double.valueOf(total_plus_vat_Element.getText().replaceAll("\\W", ""));
...
*/	
	...
	*/
	...
	public void Finish_Step(WebDriver driver,WebDriverWait wait) throws InterruptedException, ClassNotFoundException, SQLException
	{
		Thread.sleep(2000);
		JavascriptExecutor jse = (JavascriptExecutor)driver;
	...
	public void Save_continue_later(WebDriver driver,WebDriverWait wait) throws InterruptedException, ClassNotFoundException, SQLException
	{
		Thread.sleep(2000);
		JavascriptExecutor jse = (JavascriptExecutor)driver;
		WebElement save_later=wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[contains(text(),'حفظ')]")));
		jse.executeScript("scroll(0, 7500)");
		save_later.click();
	}
	
	public String get_contract_ID(WebDriver driver,WebDriverWait wait) throws InterruptedException, ClassNotFoundException, SQLException
	{
		Thread.sleep(2000);
		resv_name_list=wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.xpath("//*[contains(text(),'رقم الطلب')]/..//strong")));
		System.out.println(resv_name_list.get(0).getText());
		
		return resv_name_list.get(0).getText();
	}
	@AfterClass
