...
public class Adding_Partners {
	...
	public void Add_Partners(int cmp_type_tawsyia, WebDriver driver,WebDriverWait wait) throws InterruptedException, ClassNotFoundException, SQLException
	{
		...
		int selected_saudi_index=rand.nextInt(retrived_IDs.length-1);
		
	//	Thread.sleep(1000);
		
		for(int partners_iterator=0;partners_iterator<1;partners_iterator++)
		{
		...
		ID_field.sendKeys(retrived_IDs[selected_saudi_index]);
		
		WebElement DB_field=wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[contains(text(),'تاريخ الميلاد')]/../input")));
		DB_field.sendKeys(retrived_BDatess[selected_saudi_index].substring(8, 10)+"/"+retrived_BDatess[selected_saudi_index].substring(5, 7)+"/"+retrived_BDatess[selected_saudi_index].substring(0, 4));
